...
const SupportPage = () => {
	...
	return (
		<div className="min-h-full flex-1 flex flex-col justify-between">
			...
			{/* Hero Section */}
			<section className="relative overflow-hidden pt-4 md:pt-12 pb-16 px-4">
				<div className="max-w-5xl mx-auto text-center relative z-10">
					<motion.div
						initial={{ scale: 0.8, opacity: 0 }}
						animate={{ scale: 1, opacity: 1 }}
						transition={{ duration: 0.5 }}
						className="inline-flex items-center justify-center p-3.5 mb-5 rounded-2xl bg-cyan/10 text-cyan border border-cyan/30 shadow-[0_0_20px_-4px_rgba(0,212,255,0.3)]"
					>
			...
			<section className="max-w-6xl mx-auto px-4 mt-2 relative z-20">
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
					{["getting-started", "features", "billing", "advanced"].map(
						(cat, idx) => (
							...
							>
								<Card className="h-full border border-white/10 glass shadow-xl hover:border-cyan/40 transition-all group overflow-hidden rounded-2xl">
									<CardHeader className="pb-2">
										<div className="p-2.5 rounded-xl bg-cyan/10 text-cyan border border-cyan/20 w-fit mb-2">
											{getCategoryIcon(cat)}
										</div>
												...
												.map((article) => (
													<li key={article.id}>
														<button
															...
															className="text-xs text-white/60 hover:text-cyan transition-colors flex items-center gap-1 group/item"
														>
															<ChevronRight className="w-3 h-3 opacity-0 group-hover/item:opacity-100 -ml-4 group-hover/item:ml-0 transition-all text-cyan" />
															{article.title}
														</button>
													</li>
												))}
				...
				<Tabs defaultValue="new" className="w-full">
					<div className="flex items-center justify-between mb-8">
						<TabsList className="bg-white/[0.04] border border-white/10 p-1 rounded-xl">
							<TabsTrigger value="new" className="gap-2 rounded-lg data-[state=active]:bg-white/10 data-[state=active]:text-white">
								<Send className="w-4 h-4 text-cyan" />
								{t("form.title")}
							</TabsTrigger>
							<TabsTrigger value="history" className="gap-2 rounded-lg data-[state=active]:bg-white/10 data-[state=active]:text-white">
								<History className="w-4 h-4 text-cyan" />
								{t("history.title")}
								{tickets && tickets.length > 0 && (
									<span className="ml-1 px-1.5 py-0.5 rounded-full bg-cyan/20 border border-cyan/30 text-[10px] text-cyan font-bold font-mono">
										{tickets.length}
									</span>
								)}
								{totalUnreadCount > 0 && (
					...
					<TabsContent value="new">
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
							...
							>
								<Card className="border border-white/10 glass shadow-xl overflow-hidden rounded-2xl">
									<CardHeader>
										...
										<form onSubmit={handleSubmit} className="space-y-6">
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												<div className="space-y-2">
													...
													<Input
														...
														required
														className="bg-white/[0.03] border-white/10 text-white placeholder:text-white/40 focus-visible:ring-cyan/30 rounded-xl"
													/>
												</div>
												<div className="space-y-2">
													<label className="text-sm font-medium text-white/70">
													...
													<Select name="category" defaultValue="bug" required>
														...
														<SelectContent className="bg-[#0c0d12]/95 border-white/10 text-white backdrop-blur-xl rounded-xl">
											...
											<div className="space-y-2">
												...
												<Textarea
													...
													className="min-h-[150px] resize-none bg-white/[0.03] border-white/10 text-white placeholder:text-white/40 focus-visible:ring-cyan/30 rounded-xl"
													required
												/>
											</div>
											...
											<div className="space-y-2">
												...
												<div className="flex items-center gap-4">
													<Input
														...
														className="bg-white/[0.03] border-dashed border-white/10 text-white rounded-xl cursor-pointer"
													/>
													{screenshot && (
											...
											<Button
												type="submit"
												className="w-full md:w-auto px-8 h-12 text-base font-semibold rounded-xl bg-gradient-to-r from-azure to-cyan text-white shadow-[0_0_20px_-5px_rgba(0,212,255,0.7)] hover:shadow-[0_0_28px_-3px_rgba(0,212,255,1)] hover:brightness-110 cursor-pointer transition-all"
							...
							>
								<div>
									...
									>
										<AccordionItem
											value="item-1"
											className="border border-white/10 rounded-xl px-4 glass mb-2"
										>
											<AccordionTrigger className="hover:no-underline text-left text-white/90 hover:text-cyan text-sm">
												{t("faq.q1")}
											</AccordionTrigger>
											<AccordionContent className="text-white/60 text-xs leading-relaxed">
												{t("faq.a1")}
											</AccordionContent>
										</AccordionItem>
										<AccordionItem
								...
								<div className="p-6 rounded-2xl glass border border-white/10 shadow-xl">
									<h4 className="font-bold text-white mb-2">{t("ai_helper.title")}</h4>
									<p className="text-sm text-white/60 mb-4 leading-relaxed">
										{t("ai_helper.desc")}
									</p>
									<Button
										variant="outline"
										className="w-full group rounded-xl border-white/10 bg-white/[0.04] text-white hover:bg-white/[0.08] hover:text-white transition-all cursor-pointer"
										onClick={() =>
									...
									>
										...
										>
											<ChevronRight className="ml-2 w-4 h-4 text-cyan" />
										</motion.span>
									</Button>
								</div>
							</motion.div>
						...
						<div className="grid grid-cols-1 gap-4">
							{isLoadingTickets ? (
								Array.from({ length: 3 }).map((_, i) => (
									<div
										key={i}
										className="animate-pulse glass border border-white/10 h-24 rounded-2xl"
									/>
								))
							) : tickets && tickets.length > 0 ? (
								<AnimatePresence mode="popLayout">
									{tickets.map((ticket: SupportTicket, idx: number) => (
										...
										>
											<Card
												className="hover:border-cyan/40 transition-all glass border border-white/10 shadow-md group cursor-pointer rounded-2xl p-1"
											...
											>
												<CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
													<div className="space-y-1">
														...
														<CardDescription className="flex items-center gap-4 text-xs text-white/50">
															...
															<span className="capitalize px-2 py-0.5 rounded-lg bg-white/[0.04] border border-white/10 text-white/70">
																{ticket.category}
															</span>
														</CardDescription>
													</div>
												...
												<CardContent>
													...
													{ticket.context && (
														<div className="mt-3 flex items-center gap-2 text-[10px] text-cyan/70 font-mono">
								...
								<div className="text-center py-20 glass rounded-3xl border-2 border-dashed border-white/10">
									<div className="inline-flex p-4 rounded-full bg-white/[0.04] border border-white/10 text-cyan mb-4">
			...
			{(() => {
				...
				return (
					...
					>
						<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col p-6 glass border border-white/10 rounded-2xl shadow-2xl backdrop-blur-2xl text-white">
							<DialogHeader className="pb-4 border-b border-white/10">
								<div className="flex items-center gap-2 mb-2">
									{getStatusBadge(activeTicket.status)}
									<Badge variant="outline" className="capitalize border-white/10 text-white/80">
							...
							<div className="flex-1 py-4 space-y-6 overflow-y-auto max-h-[50vh] pr-1">
								...
								<div className="p-4 rounded-xl glass border border-white/10">
									<h4 className="text-xs font-bold text-cyan uppercase tracking-wider mb-2">
										{"Issue Description"}
									</h4>
								...
								{activeTicket.screenshot && (
									<div className="space-y-2">
										<h4 className="text-xs font-bold text-white/60 uppercase tracking-wider flex items-center gap-1.5">
											<ImageIcon className="w-3.5 h-3.5 text-cyan" />
											{"Attached Screenshot"}
										</h4>
										<div className="rounded-xl overflow-hidden border border-white/10 bg-white/[0.02] flex justify-center max-h-[300px]">
											<img
									...
									<div className="space-y-2">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
											{Boolean((activeTicket.context as Record<string, unknown>)
												.strategyId) && (
												<div className="p-3 glass border border-white/10 rounded-xl">
													<span className="text-white/50 block mb-0.5">
														{"Strategy ID"}
													</span>
								...
								<div className="space-y-4 pt-2">
									...
									<div className="space-y-3">
										{messages && messages.length > 0 ? (
												...
												>
													<div
														className={`p-3.5 rounded-2xl text-sm leading-relaxed max-w-[85%] ${
															msg.isAdmin
																? "glass border border-white/10 text-white rounded-tl-none shadow-md"
																: "bg-gradient-to-r from-azure to-cyan text-white shadow-lg rounded-tr-none"
														}`}
													>
														{msg.text && (
														...
														{msg.image && (
															<div className="mt-2 rounded-xl overflow-hidden border border-white/10 max-h-[220px] bg-black/20 flex justify-center">
																<img
																	src={msg.image}
																	alt="Attached"
																	className="max-w-full h-auto object-contain cursor-zoom-in hover:scale-[1.01] transition-transform"
													...
													<div className="flex items-center gap-1.5 mt-1 px-1 text-[10px] text-white/40 font-medium">
											...
											<div className="text-center py-6 text-xs text-white/50 glass border border-dashed border-white/10 rounded-xl">
							...
							<div className="pt-4 border-t border-white/10 space-y-2">
								{replyImage && (
									<div className="relative w-20 h-20 rounded-xl overflow-hidden border border-white/10 bg-white/[0.04] group">
										<img
											src={replyImage}
											alt="Preview"
								...
								>
									...
									<Button
										...
										variant="ghost"
										size="icon"
										className="h-10 w-10 text-white/60 hover:text-white shrink-0 rounded-xl border border-white/10 bg-white/[0.04] hover:bg-white/[0.08]"
									...
									<Textarea
										...
										className="min-h-[44px] max-h-[120px] text-xs resize-none bg-white/[0.03] border-white/10 text-white placeholder:text-white/40 focus-visible:ring-cyan/30 rounded-xl flex-1"
										required={!replyImage}
										onKeyDown={(e) => {
									...
									<Button
										...
										className="h-10 px-4 shrink-0 rounded-xl bg-gradient-to-r from-azure to-cyan text-white shadow-lg cursor-pointer"
									>
										{sendMessage.isPending ? (
											"Sending..."
										) : (
...
const QuickLinkCard: React.FC<QuickLinkCardProps> = ({
	...
	return (
		<motion.div variants={variants}>
			...
			>
				<Card className="h-full border border-white/10 glass shadow-xl hover:border-cyan/40 hover:-translate-y-1 transition-all duration-300 rounded-2xl overflow-hidden">
					<CardHeader className="pb-2">
						<div
							className={`mb-3 p-3 rounded-xl bg-cyan/10 border border-cyan/20 group-hover:border-cyan/40 transition-colors w-fit ${color}`}
						>
							{icon}
						</div>
						<CardTitle className="text-lg text-white group-hover:text-cyan transition-colors">
							{title}
						</CardTitle>
					</CardHeader>
					<CardContent>
