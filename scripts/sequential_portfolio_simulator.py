...
class AdaptiveRiskManager:
    ...
    def get_risk_multiplier(
        self, strategy_symbol: str, current_time: datetime
    ) -> float:
    ...
    def update_performance(
        self, strategy_symbol: str, trade_result: Dict, current_time: datetime
    ):
    ...
    def _adjust_risk(self, strategy_symbol: str, current_time: datetime):
        ...
        history = self.performance_history[strategy_symbol][
        ...
        total_risk = sum(
            t.get("risk_amount", t.get("planned_risk", 1.0)) for t in history
        )
        ...
        should_decrease = (
            pnl_vs_risk_pct < self.config.pnl_threshold_pct
            or win_rate < self.config.win_rate_threshold_pct
            or self.consecutive_losses[strategy_symbol]
        ...
        if strategy_symbol in self.last_penalty_time:
            time_since_penalty = (
                current_time - self.last_penalty_time[strategy_symbol]
            ).total_seconds()
            in_cooldown = time_since_penalty < self.config.cooldown_seconds

        should_increase = not in_cooldown and (
            self.consecutive_wins[strategy_symbol]
            >= self.config.recovery_consecutive_wins
            or pnl_vs_risk_pct > self.config.recovery_pnl_threshold_pct
        )
    ...
    def _get_multiplier_index(self, multiplier: float) -> int:
        ...
        try:
            ...
            # Find nearest
            return min(
                range(len(self.config.risk_multipliers)),
...
class SequentialPortfolioSimulator:
    ...
    def load_trades_from_pickle(
        self, pickle_path: str = "dashboard_data.pkl", strategies: List[str] = None
    ) -> pd.DataFrame:
    ...
    def simulate(self, trades_df: pd.DataFrame) -> Dict:
        ...
        print(
            f"   Available with leverage: ${self.config.initial_capital * self.config.leverage:.2f}"
        )
        print(f"   Max positions: {self.config.max_concurrent_positions}")
        print(
            f"   Base risk: {self.config.base_risk_pct}% (${self.config.initial_capital * self.config.base_risk_pct / 100:.2f} per trade)"
        )
        print(f"   Leverage: {self.config.leverage}x")
        print(
            f"   Adaptive risk: {'ON' if self.config.adaptive_risk_enabled else 'OFF'}"
        )
    ...
    def _open_position(self, trade_opp: Dict):
        ...
        risk_multiplier = self.risk_manager.get_risk_multiplier(
            strategy_symbol, trade_opp["entry_time"]
        )

        # Apply emergency brake
        current_dd = (
            ((self.capital - self.peak_capital) / self.peak_capital * 100)
        ...
        if (
            position_size < ideal_position_size * 0.5
        ):  # If we can open less than 50% of desired
        ...
        position = {
            ...
            "exit_price": trade_opp.get(
                "exit_price",
                trade_opp.get("entry_price", 1.0) * (1 + trade_opp.get("pnl_pct", 0.0)),
            ),
    ...
    def _close_finished_positions(self, current_time=None):
        ...
        for position in to_close:
            ...
            commission_amount = position["position_size"] * position.get(
                "commission_pct", 0.0024
            )
    ...
    def _calculate_statistics(self) -> Dict:
        ...
        avg_win_pct = (
            (wins["pnl_pct"].mean() * 100) if len(wins) > 0 else 0
        )  # pnl_pct is in decimal form
        avg_loss_pct = abs((losses["pnl_pct"].mean() * 100)) if len(losses) > 0 else 0

        profit_factor = (
            (wins["pnl_amount"].sum() / abs(losses["pnl_amount"].sum()))
            if len(losses) > 0 and losses["pnl_amount"].sum() != 0
            else 0
        )
        ...
        max_win_streak = (
            streaks[streaks["first"]]["count"].max()
            if len(streaks[streaks["first"]]) > 0
            else 0
        )
        max_loss_streak = (
            streaks[~streaks["first"]]["count"].max()
...
def plot_results(stats: Dict, config: SimulatorConfig):
    ...
    trades_df["risk_ma"] = (
        trades_df["risk_multiplier"].rolling(window=20, min_periods=1).mean()
    )
    fig.add_trace(
        go.Scatter(
    ...
    trades_df["streak_change"] = (
        trades_df["is_win"] != trades_df["is_win"].shift()
    ).cumsum()

    streaks = (
        trades_df.groupby("streak_change")
        .agg({"is_win": "first", "exit_time": "first", "pnl_amount": "size"})
        .reset_index()
    )
...
def compare_strategies():
    ...
    for strategy in strategies:
        ...
        try:
            ...
            if stats:
                ...
                print(
                    f"   ✅ Final Capital: ${stats['final_capital']:.2f} ({stats['total_pnl_pct']:.2f}%)"
                )
    ...
    print("✅ Comparison saved to sequential_comparison.csv")
...
def save_sequential_dashboard_data(results: Dict, config: SimulatorConfig):
    ...
    for asset_name in all_assets:
        ...
        for strategy_name, stats in results.items():
            ...
            win_rate = (
                (len(wins) / len(asset_trades) * 100) if len(asset_trades) > 0 else 0
            )
...
def run_simulation_interface(
    assets_data: Dict, source_strategy_name: str, config_dict: Dict
) -> Optional[Dict]:
    """
    ...
    for asset_name, asset_data in assets_data.items():
        ...
        for trade in trades:
            ...
            if "exit_price" not in trade_copy:
                trade_copy["exit_price"] = trade_copy["entry_price"] * (
                    1 + trade_copy.get("pnl_pct", 0.0)
                )
    ...
    df = df.sort_values(
        by=["entry_time", "volatility"], ascending=[True, False]
    ...
    result = {"summary": summary, "stats": stats, "config": config}
