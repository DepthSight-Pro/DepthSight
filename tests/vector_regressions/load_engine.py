...
nodes = [
    ...
    and n.name == "FastVectorBacktester"
    or isinstance(n, ast.FunctionDef)
    and "funding" in n.name
]
ns = dict(
    vars(typing),
    pd=pd,
    ...
    config=SimpleNamespace(BACKTEST_FUNDING_RATE_8H=0, PHANTOM_TRACKING_ENABLED=False),
...
def sim(
    ...
    x.main_df["ATR_14"] = 1.0
    x.signals = pd.DataFrame(
        {
            "enter_short" if side == "SHORT" else "enter_long": signals
            if signals is not None
            else [True] + [False] * (len(rows) - 1)
        },
        index=x.main_df.index,
    )
    x.strategy_json = {
        "initialization": {
            "params": dict(
    ...
    x.slippage_pct = slip
    x.commission_pct = fee
    x.initial_balance = x.current_balance = x.peak_equity = float(balance)
    x.total_pnl_usd = x.total_commission_usd = x.max_drawdown = x.max_floating_dd = 0.0
    x.equity_curve = []
    x._is_liquidated = False
    x.is_trading_allowed = True
    x.trade_start_ts = None
    x.symbol = "SYNTHETIC"
    x.strategy_name = "AUDIT"
    x.use_oracle = False
    x.exchange_info = {}
    x.config = ns["config"]
    x.structured_report = {
        "event_counters": {"rejections": defaultdict(int), "trades_opened": 0}
    }
