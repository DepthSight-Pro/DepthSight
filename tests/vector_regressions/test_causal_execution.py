...
class CausalExecutionTests(unittest.TestCase):
    def test_future_and_invalid_shifts_rejected(self):
        for shift in (-1, -0.5, 0.5, "bad", float("nan"), float("inf")):
            with self.subTest(shift=shift), self.assertRaises(ValueError):
                C._validate_causal_shifts(
                    {"children": [{"source": "candle", "shift": shift}]}
                )
    ...
    def test_runtime_resolver_also_rejects_future(self):
        with self.assertRaises(ValueError):
            C.__new__(C)._resolve_value_series(
                {"source": "candle", "key": "close", "shift": -1}
            )
    ...
    def test_partial_then_full_target_uses_remaining_quantity(self):
        x = sim(
            [FLAT, [100, 200, 99, 100], FLAT],
            qty=10,
            balance=1000,
            tp=20,
            extra={
                "partial_exits": [
                    {"size_pct": 50, "tp_type": "percent_from_price", "tp_value": 10}
                ]
            },
        )
    ...
    def test_fees_and_slippage_conservation(self):
        ...
        entry, exit = 100 * 1.0005, 110 * 0.9995
        fee = (entry + exit) * 2 * 0.0006
    ...
    def test_end_of_data_still_reconciles_full_equity(self):
        ...
        self.assertAlmostEqual(k["total_return_all"], -0.01)

    def test_atr_helper_preserves_warmup_nan(self):
        b = C.__new__(C)
        b.main_df = pd.DataFrame({"ATR_14": [np.nan, np.nan, 7.0]})
    ...
    def test_sma_warmup_is_prefix_invariant(self):
        ...
        ns["ta"] = SimpleNamespace(
            sma=lambda close, length: close.rolling(length).mean()
        )
        try:
            for future_close in (100.0, 400.0):
                b = C.__new__(C)
                b.main_df = pd.DataFrame(
                    {"close": [100.0, 100.0, future_close], "ATR_14": [1.0, 1.0, 1.0]}
                )
                b.base_timeframe = "1m"
                ...
                b.strategy_json = {
                    "entryConditions": {
                        ...
                        "params": {
                            "leftOperand": {"source": "indicator", "key": "SMA_3"}
                        },
                    }
                }
                ...
                self.assertAlmostEqual(
                    b.signals.SMA_3.iloc[2], (200 + future_close) / 3
                )
        finally:
            ns["ta"] = original_ta
    ...
    def test_closed_hour_does_not_change_with_future(self):
        ...
        b.main_df = pd.DataFrame({"close": 100.0}, index=idx)
        b.base_timeframe = "1h"
        b.data_context = {
            "1h": pd.DataFrame({"close": [100.0, 110.0, 120.0]}, index=idx[::60])
        }
