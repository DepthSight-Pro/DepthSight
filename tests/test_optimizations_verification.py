...
from bot_module.depthsight_backtester import count_funding_periods, calculate_position_funding_pnl
...
def test_calculate_position_funding_pnl():
    execs = [
        {"timestamp": datetime(2026, 7, 1, 7, 0, tzinfo=timezone.utc), "price": 50000.0, "quantity": 1.0, "type": "ENTRY"},
        {"timestamp": datetime(2026, 7, 1, 17, 0, tzinfo=timezone.utc), "price": 51000.0, "quantity": 1.0, "type": "EXIT"},
    ]
    ...
    funding_pnl = calculate_position_funding_pnl(execs, is_short=False, funding_rate=0.0001)
    assert pytest.approx(funding_pnl) == -10.0
    
    # Short position, receives funding rate of 0.0001 per period
    # expected_funding_fee = 1.0 * 2 * 50000.0 * 0.0001 = +10.0
    funding_pnl_short = calculate_position_funding_pnl(execs, is_short=True, funding_rate=0.0001)
