...
export const MemoryBank: React.FC<MemoryBankProps> = ({
	...
	const [isCommunityOptIn, setIsCommunityOptIn] = useState(
	...
	const handleToggleCommunity = async (enabled: boolean) => {
		...
		try {
			const res = await api.updateCommunityMemorySharing(enabled);
			if (enabled && res.promoted_count && res.promoted_count > 0) {
				alert(
					t(
			...
			console.error("Failed to update community memory sharing", e);
			setIsCommunityOptIn(!enabled);
			if (updateUser) {
				updateUser({
					shareCommunityMemories: !enabled,
	...
	const handleShareMemory = async (memoryId: string) => {
		...
		try {
			await api.shareAgentMemory(memoryId);
			alert(
				t(
	...
	const handleModalAccept = () => {
		...
		void handleToggleCommunity(true);
	};
	...
	const handleDeduplicateMemories = async () => {
		...
		try {
			...
			alert(
				`Successfully reorganized memories! Merged ${res.deleted_count} duplicate insights.`
			);
			void fetchMemories();
		} catch (e) {
	...
	return (
		<div className="flex flex-col h-full bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-2xl overflow-hidden p-3.5 shadow-xl relative">
			{/* Header */}
			<div className="flex items-center justify-between border-b border-slate-800 pb-2.5 mb-2.5">
				...
				<div className="flex items-center gap-1">
					...
					<button
						...
						disabled={isLoading}
						className="p-1.5 hover:bg-slate-800 rounded text-slate-400 transition disabled:opacity-50"
					...
					<button
						...
						className="p-1.5 hover:bg-slate-800 hover:text-amber-400 rounded text-slate-400 transition disabled:opacity-50"
						title="Reorganize Memory Bank"
						type="button"
					>
						<Sparkles
							className={`w-4 h-4 ${
								isDeduplicating ? "animate-pulse" : ""
							}`}
						/>
					</button>
					<button
						...
						className="p-1.5 hover:bg-slate-800 hover:text-rose-400 rounded text-slate-400 transition disabled:opacity-50"
			...
			<div className="mb-2.5 p-2.5 rounded-xl border border-indigo-500/25 bg-gradient-to-r from-indigo-950/50 via-slate-900 to-indigo-950/30 flex items-center justify-between gap-2.5 shadow-inner">
				...
				<label className="relative inline-flex items-center cursor-pointer shrink-0">
					<input
						...
						onChange={(e) => void handleToggleCommunity(e.target.checked)}
			...
			{/* Filter Tabs & Tag Chips */}
			{showMemories && (
				<div className="space-y-2 pb-2 mb-2 border-b border-slate-800/60">
					{/* Scope Selector */}
							...
							<div
								...
								className={`group relative border ${
									isNew
										...
										: ""
								} ${cardStyles} rounded-xl p-3 transition-all duration-300 shadow-sm`}
							>
								...
								<div className="text-xs text-slate-300 leading-relaxed font-light space-y-1.5">
								...
								{/* Card Actions */}
								<div className="absolute top-2 right-2 flex items-center gap-1 bg-slate-900/90 px-1.5 py-0.5 rounded border border-slate-850 shadow-md">
									<span className="text-[9px] text-slate-400 font-mono">
									...
									{isCommunity ? (
										...
										<>
											<button
												onClick={(e) => {
													e.stopPropagation();
													void handleShareMemory(memory.id);
												}}
			...
			{showOnboardingModal &&
				...
				createPortal(
					<div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
						...
						/>
						<div className="relative z-10 bg-slate-900 border border-indigo-500/30 rounded-2xl p-4 sm:p-5 w-full max-w-sm sm:max-w-md shadow-2xl relative max-h-[85dvh] sm:max-h-[85vh] flex flex-col my-auto">
							<button
								onClick={handleModalDecline}
								className="absolute top-3 right-3 text-slate-400 hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-800 transition z-10"
								type="button"
							>
								<X className="w-5 h-5" />
							</button>

							<div className="flex items-center gap-3 pr-7 shrink-0 pb-2">
								<div className="p-2.5 bg-indigo-500/20 text-indigo-400 rounded-xl shadow-inner border border-indigo-500/30 shrink-0">
								...
								<div className="min-w-0">
									...
									<p className="text-[10px] sm:text-[11px] text-indigo-400 mt-0.5">
										{t(
							...
							<div className="flex-1 min-h-0 overflow-y-auto space-y-2.5 pr-1 py-1 text-xs text-slate-300 scrollbar-thin">
								...
								<div className="bg-slate-950/60 rounded-xl p-2.5 sm:p-3 border border-slate-800 space-y-2 text-[10px] sm:text-[11px] text-slate-300">
							...
							<div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-2.5 mt-2 border-t border-slate-800/80 shrink-0">
								<button
									type="button"
									onClick={handleModalDecline}
									className="w-full sm:w-auto px-3.5 py-2 rounded-xl text-xs font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition text-center"
								>
								...
								<button
									...
									className="w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-600/25 transition flex items-center justify-center gap-1.5"
								>
