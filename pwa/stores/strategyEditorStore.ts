// pwa/stores/strategyEditorStore.ts

import { produce } from "immer";
import { v4 as uuidv4 } from "uuid";
import { create } from "zustand";
import i18n from "../i18n";
import { migrateStrategy } from "../services/strategyMigration";
import type { StrategyConfigData } from "../types";
import {
	type ClosePositionBlock,
...
const identifyAndInitializeCompositeBlock = (block: ConditionBlock) => {
	if (block.type === "AND" && block.children && block.children.length > 0) {
		// Check for tape_condition pattern
		if (
			block.children.length === 2 &&
			block.children[0].type === "tape_analysis" &&
			block.children[1].type === "value_comparison"
		) {
			block.compositeType = "tape_condition";
			block.isComposite = true;
			block.displayMode = "simplified";
			console.log(
				`[loadState] Identified composite block: ${block.id} as tape_condition`,
			);
		}
		// Check for order_book_zone_condition pattern
		else if (
			block.children.length === 2 &&
			block.children[0].type === "order_book_zone" &&
			block.children[1].type === "value_comparison"
		) {
			block.compositeType = "order_book_zone_condition";
			block.isComposite = true;
			block.displayMode = "simplified";
			console.log(
				`[loadState] Identified composite block: ${block.id} as order_book_zone_condition`,
			);
		}
		// Check for level_proximity_condition pattern
		else if (
			block.children.length === 2 &&
			block.children[0].type === "local_level" &&
			block.children[1].type === "value_comparison"
		) {
			...
			block.displayMode = "simplified";
			console.log(
				`[loadState] Identified composite block: ${block.id} as level_proximity_condition`,
			);
		}
	}
};
...
const traverseBlocks = (
	...
	callback: (block: ConditionBlock | ManagementBlock) => void,
) => {
	if (!blocks) return;

	const processBlock = (block: ConditionBlock | ManagementBlock) => {
		callback(block);
		if ("children" in block && block.children) {
			block.children.forEach((child) => processBlock(child));
		}
		if ("if_conditions" in block && block.if_conditions) {
			processBlock(block.if_conditions as ConditionBlock);
		}
		if ("then_actions" in block && block.then_actions) {
			(block.then_actions as (ConditionBlock | ManagementBlock)[]).forEach(
				(action) => processBlock(action),
			);
		}
	};
		...
		if (
			...
			for (let i = 0; i < thenActions.length; i++) {
				...
				if (action.id === blockId)
					return {
						parent: root,
						block: action as unknown as ManagementBlock,
						index: i,
					};
			}
		}
...
const findBlockRecursive = (
	...
	blockId: string,
): ConditionBlock | ManagementBlock | null => {
	if (!root) return null;
	...
	if (block.type === "conditional_management") {
		...
		return {
			...
			then_actions: conditionalBlock.then_actions.map(
				(action) =>
					serializeManagementBlock(action as ManagementBlock) as unknown as
						| TrailingStopBlock
						| MoveToBeBlock
						| ModifyStopLossBlock
...
const getDefaultInitialState = (): StrategyState => ({
	...
	name: i18n.t("editor.defaultStrategyName"),
	description: "",
	symbol: "BTCUSDT",
	marketType: "FUTURES",
...
export type StateKey = "filters" | "entryConditions" | "positionManagement";

// --- FULL ACTIONS INTERFACE ---
interface EditorActions {
	toJson: () => StrategyConfigData;
	loadState: (strategy: Record<string, unknown>) => void;
	loadStrategy: (strategy: Record<string, unknown>) => void;
...
export const useStrategyEditorStore = create<StrategyState & EditorActions>(
	(set, get) => ({
		...
		toJson: (): StrategyConfigData => {
			...
			return {
				name: state.name,
				strategy_name: resolveRuntimeStrategyName(state.strategy_name, state),
				signal_source: state.signal_source,
				symbol: state.symbol,
				marketType: state.marketType,
				min_foundation_weight_threshold: state.useFoundationWeights
					? state.min_foundation_weight_threshold
					: 0,
				foundation_weights: state.useFoundationWeights
				...
				natr_settings:
					state.symbol_selection_mode === "DYNAMIC_NATR"
						? { min_natr: state.min_natr }
						: null,
				oracle_settings:
					state.symbol_selection_mode === "DYNAMIC_ORACLE"
						? { regime: state.oracleRegime, confidence: state.oracleConfidence }
						: null,
				max_concurrent_symbols: state.max_concurrent_symbols,
			};
		},
		...
		loadState: (strategy) =>
			set(
				produce((draft: StrategyState) => {
					...
					const configData = (strategyObj.config_data as Record<string, unknown>) || {};
					...
					console.log(
						"[loadState] Flattened Data after migration:",
					...
					draft.symbol =
						...
						(Array.isArray(strategyObj.symbols)
							...
							: null) ||
						(strategyObj.symbol as string) ||
						defaults.symbol;
					draft.marketType =
						(flattenedData.marketType as "FUTURES" | "SPOT") ??
						(flattenedData.market_type as "FUTURES" | "SPOT") ??
						(configData.marketType as "FUTURES" | "SPOT") ??
						defaults.marketType;
					draft.min_foundation_weight_threshold =
						(flattenedData.min_foundation_weight_threshold as number) ??
						(strategyObj.min_foundation_weight_threshold as number) ??
						defaults.min_foundation_weight_threshold;
					draft.oracleRegime =
						(flattenedData.oracle_regime as number) ??
						(strategyObj.oracle_regime as number) ??
						defaults.oracleRegime;
					draft.oracleConfidence =
						(flattenedData.oracle_confidence as number) ??
						(strategyObj.oracle_confidence as number) ??
						defaults.oracleConfidence;
					draft.use_ml_confirmation =
						(flattenedData.use_ml_confirmation as boolean) ??
						(strategyObj.use_ml_confirmation as boolean) ??
						defaults.use_ml_confirmation;
					draft.breakeven_on_regime_change =
						(flattenedData.breakeven_on_regime_change as boolean) ??
						(strategyObj.breakeven_on_regime_change as boolean) ??
						defaults.breakeven_on_regime_change;

					let mode =
						(strategyObj.symbol_selection_mode as string) ||
						defaults.symbol_selection_mode;
					if (mode === "DYNAMIC") {
						mode =
							flattenedData.oracle_settings ||
							strategyObj.oracle_regime !== undefined
								? "DYNAMIC_ORACLE"
								: "DYNAMIC_NATR";
					}
					draft.symbol_selection_mode = mode as
						| "STATIC"
						| "DYNAMIC_NATR"
						| "DYNAMIC_ORACLE";
					draft.max_concurrent_symbols =
						(flattenedData.max_concurrent_symbols as number) ??
						(strategyObj.max_concurrent_symbols as number) ??
						defaults.max_concurrent_symbols;
					draft.min_natr =
						(flattenedData.natr_settings as { min_natr?: number })?.min_natr ??
						(strategyObj.min_natr as number) ??
						defaults.min_natr;
					...
					const weights =
						...
						(strategyObj.foundationWeights as Record<string, number>) ||
						(configData.foundationWeights as Record<string, number>) ||
						(configData.foundation_weights as Record<string, number>) ||
						(flattenedData.foundation_weights as Record<string, number>);
					draft.foundationWeights = normalizeFoundationWeights(weights);
					if (Object.keys(draft.foundationWeights).length > 0) {
						draft.useFoundationWeights = true;
					}

					// Initialize displayMode for composite blocks
					traverseBlocks(draft.filters, (block) => {
						identifyAndInitializeCompositeBlock(block as ConditionBlock);
						console.log(
							`[loadState] Processing block: ${block.id} (${block.type}), compositeType: ${"compositeType" in block ? block.compositeType : "N/A"}, displayMode: ${"displayMode" in block ? block.displayMode : "N/A"}`,
						);
					});
					traverseBlocks(draft.entryConditions, (block) => {
						identifyAndInitializeCompositeBlock(block as ConditionBlock);
						console.log(
							`[loadState] Processing block: ${block.id} (${block.type}), compositeType: ${"compositeType" in block ? block.compositeType : "N/A"}, displayMode: ${"displayMode" in block ? block.displayMode : "N/A"}`,
						);
					});
					traverseBlocks(draft.positionManagement, (block) => {
						// Management blocks can also be composite, but their structure is different.
						// For now, we only identify ConditionBlocks as composite.
						// If composite management blocks are introduced, this logic will need to be extended.
						if (block.type !== "conditional_management") {
							// Conditional management blocks have their own internal structure
							identifyAndInitializeCompositeBlock(block as ConditionBlock);
						}
						console.log(
							`[loadState] Processing block: ${block.id} (${block.type}), compositeType: ${"compositeType" in block ? block.compositeType : "N/A"}, displayMode: ${"displayMode" in block ? block.displayMode : "N/A"}`,
		...
		setStrategyField: (field, value) => set({ [field]: value } as unknown as Partial<StrategyState & EditorActions>),
		setTrigger: (trigger) =>
			set(
				produce((draft: StrategyState) => {
			...
			set(
				produce((draft: StrategyState) => {
					...
					if (blockToInsert) {
						...
						newBlock = createCompositeBlock(type as NonNullable<ConditionBlock["compositeType"]>);
					} else {
						newBlock = createNewBlock(type) as ConditionBlock;
					}
					const targetRoot =
						target === "filters" ? draft.filters : draft.entryConditions;
					if (
						!parentId ||
						parentId.includes("root") ||
						parentId.includes("drop-zone")
					) {
						targetRoot.children?.push(newBlock);
					} else {
						const added = addBlockToParent(targetRoot, parentId, newBlock);
						if (!added) {
							targetRoot.children?.push(newBlock);
						}
					}
		...
		findBlock: (blockId) => {
			const state = get();
			let block =
				findBlockRecursive(state.filters, blockId) ||
				findBlockRecursive(state.entryConditions, blockId);
			if (block) return block;
			for (const pmBlock of state.positionManagement) {
				block = findBlockRecursive(pmBlock, blockId);
				if (block) return block;
			}
			return null;
		},
		...
		updateBlockParams: (blockId, newParams) =>
			set(
				produce((draft) => {
					const findBlockInDraft = (
						id: string,
					): ConditionBlock | ManagementBlock | null => {
						let block =
							findBlockRecursive(draft.filters, id) ||
							findBlockRecursive(draft.entryConditions, id);
						if (block) return block;
						for (const pmBlock of draft.positionManagement) {
							block = findBlockRecursive(pmBlock, id);
							if (block) return block;
						}
						return null;
					};
					const block = findBlockInDraft(blockId);
					if (block?.params) {
		...
		updateBlockDisplayMode: (blockId, displayMode) =>
			set(
				produce((draft) => {
					const findBlockInDraft = (
						id: string,
		...
		moveCondition: (target, activeId, overId) =>
			set(
				produce((draft) => {
					const findItemsAndParent = (
						root: ConditionBlock,
						id1: string,
						id2: string,
					) => {
						const { parent: parent1 } = findBlockAndParentRecursive(root, id1);
						const { parent: parent2 } = findBlockAndParentRecursive(root, id2);
						if (
							parent1 &&
							parent2 &&
					...
					let items: (ConditionBlock | ManagementBlock)[] | null = null;
					if (target === "filters")
						items = findItemsAndParent(draft.filters, activeId, overId);
					else if (target === "entryConditions")
						items = findItemsAndParent(draft.entryConditions, activeId, overId);
					else if (target === "positionManagement") {
						const activeIndex = draft.positionManagement.findIndex(
							(b: ManagementBlock) => b.id === activeId,
						);
						const overIndex = draft.positionManagement.findIndex(
							(b: ManagementBlock) => b.id === overId,
						);
						if (activeIndex > -1 && overIndex > -1) {
							items = draft.positionManagement;
						}
					}
		...
		updateCompositeConditionParams: (blockId, newSimpleParams) =>
			set(
				produce((draft: StrategyState) => {
					...
					if (
						!parentBlock ||
						!(parentBlock as ConditionBlock).isComposite ||
						!(parentBlock as ConditionBlock).children
					) {
						console.error(
							"updateCompositeConditionParams: Block not found or not a valid composite block.",
						);
						return;
					}
					const compositeBlock = parentBlock as ConditionBlock;
					const [provider, consumer] = compositeBlock.children!;

					switch (compositeBlock.compositeType) {
						case "tape_condition":
							if (provider?.params && consumer?.params) {
								if (newSimpleParams.metric !== undefined)
									(consumer.params.leftOperand as Record<string, unknown>).key =
										normalizeTapeOutputKey(newSimpleParams.metric as string);
								if (newSimpleParams.operator !== undefined)
									consumer.params.operator = newSimpleParams.operator as string;
								if (newSimpleParams.value !== undefined)
									consumer.params.rightOperand = newSimpleParams.value as number;
								if (newSimpleParams.time_window_sec !== undefined) {
									provider.params.time_window_sec =
										newSimpleParams.time_window_sec as number;
									(consumer.params.leftOperand as Record<string, unknown>).key =
										normalizeTapeOutputKey(
											(consumer.params.leftOperand as Record<string, unknown>).key as string,
										);
								}
							}
							break;
						case "order_book_zone_condition":
							if (provider?.params && consumer?.params) {
								...
								if (newSimpleParams.side !== undefined)
									provider.params.side = newSimpleParams.side as string;
								if (newSimpleParams.range_value !== undefined)
									provider.params.range_value = newSimpleParams.range_value as number;
								if (newSimpleParams.range_type !== undefined)
									provider.params.range_type = newSimpleParams.range_type as string;
								if (newSimpleParams.operator !== undefined)
									consumer.params.operator = newSimpleParams.operator as string;
								if (newSimpleParams.value !== undefined)
									consumer.params.rightOperand = newSimpleParams.value as number;
		...
		addManagementBlock: (type, index) =>
			set(
				produce((draft) => {
					...
					else draft.positionManagement.push(newBlock);
				}),
			),
		...
		updateManagementBlockParams: (blockId, newParams) =>
			set(
				produce((draft: StrategyState) => {
					...
					if (block?.params) Object.assign(block.params, newParams);
				}),
			),
			...
			set(
				produce((draft: StrategyState) => {
					...
					if (typeof typeOrBlock !== "string") {
						...
						newBlock = createCompositeBlock(typeOrBlock as NonNullable<ConditionBlock["compositeType"]>);
					} else {
				...
				produce((draft: StrategyState) => {
					...
					for (const root of draft.positionManagement) {
						if (
							...
							if (ifRoot.id === blockId || root.id === blockId) {
		...
		updateFoundationWeight: (blockId: string, weight: number) =>
			set(
				produce((draft: StrategyState) => {
					if (!draft.foundationWeights) draft.foundationWeights = {};
		...
		setUseFoundationWeights: (value) =>
			set(
				produce((draft) => {
					...
					if (value && !draft.foundationWeights) draft.foundationWeights = {};
				}),
			),
